...
<!-- 正文开始 -->
				<div class="main-content">
					<div class="breadcrumbs" id="breadcrumbs">
						...
						<ul class="breadcrumb">
							<li>
								<i class="icon-user"></i>
								<a id="" role="button" data-toggle="modal" href="#">待处理报账申请</a>
							</li>
						</ul><!-- .breadcrumb -->
						<div class="accessBar" style="display: inline-block;">
							<div style="display: inline-block;">
								<a class="" id="reimbursementApplication" data-toggle="modal" href="#" title="付款申请">
	...
	<!-- 列表循环 -->				
								<c:forEach var="reimbursementApplication" items="${reimbursementApplications }" varStatus="status">
									<tr id="${reimbursementApplication.localTourTable.id }">
										...
										<td><a id="editTour" role="button" data-toggle="modal" href="#edit">${reimbursementApplication.localTourTable.tourNo }</a></td>
										<td>${reimbursementApplication.localTourTable.tourName }</td>
										<td>${reimbursementApplication.costSum }</td>
										<td>${reimbursementApplication.willPaySum }</td>
										<td>${reimbursementApplication.realPaySum }</td>
										<td>${reimbursementApplication.reimbursementSum }</td>
										<td>${reimbursementApplication.willIncomeSum }</td>
										<td>${reimbursementApplication.realIncomeSum }</td>
										<td>${reimbursementApplication.realGrossProfit }</td>
										<td>${reimbursementApplication.realGrossMargin }</td>
										<td>${reimbursementApplication.userRealName }</td>
										<td>${reimbursementApplication.status }</td>
									</tr>
								</c:forEach>
	<!-- 列表循环结束 -->								
							</tbody>
...
<script type="text/javascript">
	...
	/* 初始化 */
		...
		$("#toDoReimbursementApplication").addClass("active");
	...
	/*查看*/
		$("#reimbursementApplication").click(function(){
			...
			if(checkbox.length==0){
				...
				$.ajax({
			        ...
			        url:"/localtour/reimbursementApplication/find",  
			        data:myData,  
			        dataType: "json",  
			        async: false,  
		...
		$("#ok").mouseenter(function(){
			...
			$("body").append(div);
			$('#qrcode').qrcode({width: 200,height: 200,text: http+"/localtour/reimbursementApplication/ok?tourId="+tourId});
			div.fadeIn();
		});
		$("#ok").mouseout(function(){
			...
			$.ajax({
		        type: "GET",  
		        contentType:"application/json;charset=utf-8",  
		        url:"/localtour/reimbursementApplication/checkStatus", 
		        data:myData,  
		        dataType: "json",  
		        async: false,  
		        success:function(data){
		...
		$("#cancel").click(function(){
			...
			$.ajax({
		        type: "GET",  
		        contentType:"application/json;charset=utf-8",  
		        url:"/localtour/reimbursementApplication/cancel",  
		        data:myData,  
		        dataType: "json",  
		        async: false,  
		        success:function(data){
