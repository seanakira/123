...
<!-- 列表循环 -->								
							<c:forEach var="user" items="${users }" varStatus="status">
								<tr id="" <%-- <c:if test="${status.index%2!=0 }"> --%>class="style:{background-color:#f9f9f9;}"<%-- </c:if> --%>>
									...
									<td id="" class="">
										<a id="" role="button" data-toggle="modal" href="#modal-table">${user.userTable.userName }</a>
									</td>
									<td class="">${user.userTable.realName }</td>
									<td class="">
										${user.deptName }
										<input type="hidden" value="${user.userTable.deptId }"/>
									</td>
									<td class="">${user.userTable.position }</td>
									<td class="">${user.userTable.phone }</td>
									<td class="">${user.userTable.qq }</td>
