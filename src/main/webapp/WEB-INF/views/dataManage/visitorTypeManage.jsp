				...
				<div class="main-content">
					<div class="breadcrumbs" id="breadcrumbs">
						...
						<div class="accessBar" style="display: inline-block;">
							<shiro:hasPermission name="visitorType:save">
								<a class="blue" id="add" data-toggle="modal" href="#" title="新增游客类型">
						...
						<div class="nav-search" id="nav-search">
							<form class="form-search" action="${path }visitorTypeManage" method="get">
...
<!-- 增加模板结束 -->		
<!-- 列表循环 -->								
							<c:forEach var="visitorType" items="${visitorTypes }" varStatus="status">
								<tr id="" <%-- <c:if test="${status.index%2!=0 }"> --%>class="style:{background-color:#f9f9f9;}"<%-- </c:if> --%>>
									...
									<td class="">${visitorType.id }</td>
									<td class="">${visitorType.visitorTypeName }</td>
									<td class="hidden-480 " id="">
									<c:choose>
										<c:when test="${visitorType.enable }">
											<span class="label label-sm label-success">有效</span>
										</c:when>
										<c:otherwise>
									...
									<td class=" ">
										<div class="visible-md visible-lg hidden-sm hidden-xs action-buttons">
											<shiro:hasPermission name="visitorType:update">
											...
											<span>
												<c:choose>
													<c:when test='${visitorType.enable }'>
														<shiro:hasPermission name="visitorType:del">
															<a id="${visitorType.id }" class="red" href="#">
																<i class="icon-trash bigger-130"></i>
															</a>
														</shiro:hasPermission>
													</c:when>
													<c:otherwise>
														<shiro:hasPermission name="visitorType:recover">
															<a id="${visitorType.id }" class="green" href="#">
								...
								<div class="dataTables_paginate paging_bootstrap">
									<ul class="pagination">
										<li <c:choose><c:when test="${pageNo==1 }">class="prev disabled"</c:when><c:otherwise>class="prev"</c:otherwise></c:choose>>
											<a href="/localtour/visitorTypeManage?page=${pageNo-1 }&key=${key }"><i class="icon-double-angle-left"></i></a>
										</li>
										<c:forEach var="page" begin="1" end="${pageMax }">
											<li <c:if test="${pageNo==page }">class="active"</c:if>>
												<a href="/localtour/visitorTypeManage?page=${page }&key=${key }">${page }</a>
											</li>
										</c:forEach>
										<li <c:choose><c:when test="${pageNo==pageMax }">class="next disabled"</c:when><c:otherwise>class="next"</c:otherwise></c:choose>>
											<a href="/localtour/visitorTypeManage?page=${pageNo+1 }&key=${key }"><i class="icon-double-angle-right"></i></a>
										</li>
									</ul>
								</div>
							</div>
...
<script type="text/javascript">
	...
	/* 初始化 */
			...
			$("#visitorTypeManage").addClass("active");
	/* 新增 */		
			$("#add").click(function(){
	...
	/* 回车保存 */		
		$("#table").delegate("#submit","keydown",function(event){
			if(event.keyCode==13){
				...
				var input = $(this);
				var visitorTypeName = $(this).val();
				var visitorType = {visitorTypeName:visitorTypeName};
				var myData = JSON.stringify(visitorType);
			 	$.ajax({  
			        type: "POST",  
			        contentType:"application/json;charset=utf-8",  
			        url:"/localtour/visitorTypeManage/save",  
			        data:myData,  
			        dataType: "json",  
			        async: false,  
			        success:function(data){
			        	input.parent().html(visitorTypeName);
			        }  
				 });
	...
	/* 删除 */
		$("#table").delegate(".red","click",function(){
			...
			$.ajax({  
		        type: "GET",  
		        contentType:"application/json;charset=utf-8",  
		        url:"/localtour/visitorTypeManage/del",  
		        data:myData,  
		        dataType: "json",  
		        async: false,  
		        success:function(data){
	...
	/* 恢复 */
		$("#table").delegate("span .green","click",function(){
			...
			$.ajax({  
		        type: "GET",  
		        contentType:"application/json;charset=utf-8",  
		        url:"/localtour/visitorTypeManage/recover",  
		        data:myData,  
		        dataType: "json",  
		        async: false,  
		        success:function(data){
	...
	/* 编辑 */
		$("#table").delegate("#edit","click",function(){
			...
			var info = {id:td.eq(-1).children("a").attr("id"),
						visitorTypeName:td.eq(2).text()};
			var myData = JSON.stringify(info);
			td.eq(2).html("<input id='update' type='text' value='"+info.visitorTypeName+"' style='width:150px' />");
		...
		$("#table").delegate("#update","keydown",function(event){
			if(event.keyCode==13){
				...
				var params = $(this).parents("tr").find("input");
		 		var visitorTypeName = params.eq(1).val();
				var id = obj.find(".red").attr("id");
				var visitorType = {id:id,visitorTypeName:visitorTypeName};
				var myData = JSON.stringify(visitorType);
				$.ajax({  
			        type: "POST",  
			        contentType:"application/json;charset=utf-8",  
			        url:"/localtour/visitorTypeManage/update",  
			        data:myData,  
			        dataType: "json",  
			        async: false,  
			        success:function(data){
			        	params.eq(1).parent().html(visitorTypeName);
			        }  
				 }); 
