...
<script type="text/javascript">
	...
	/* 初始化 */
		...
		$("#budgetExecuteReimbursementContrast").addClass("active");
		...
		$("#find").click(function(){
			if($("#wait").length==0){
				...
				$.ajax({
			        ...
			        url:"${path }budgetExecuteReimbursementContrast/get",
			        data:myData,
			        dataType: "json",
			        async: true,
			        success:function(data){
						...
						progress.remove();
						$.each(data,function(){
							table.append('<tr><td>'+this.tourNo+'</td><td>'+this.budgetIncomeSum+'</td><td>'+this.budgetCostSum+'</td><td>'+this.budgetGrossProfit+'</td><td>'+this.budgetGrossMargin+'%</td>'+
																							   '<td class="blue">'+this.executeIncomeSum+'</td><td class="blue">'+this.executeCostSum+'</td><td class="blue">'+this.executeGrossProfit+'</td><td class="blue">'+this.executeGrossMargin+'%</td>'+
																							   '<td class="red">'+this.reimbursementIncomeSum+'</td><td class="red">'+this.reimbursementCostSum+'</td><td class="red">'+this.reimbursementGrossProfit+'</td><td class="red">'+this.reimbursementGrossMargin+'%</td>'+
																							   '<td>'+this.deptName+'</td><td>'+this.userRealName+'</td></tr>');
						});
			        	if($("#table").html()==""){
							$("#table").html("没有查询到结果");
						}
