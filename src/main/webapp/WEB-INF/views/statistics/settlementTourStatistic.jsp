...
<script type="text/javascript">
	...
	/* 初始化 */
		...
		$("#settlementTourStatistic").addClass("active");
		...
		$("#find").click(function(){
			if($("#wait").length==0){
				...
				$.ajax({
			        ...
			        url:"${path }settlementTourStatistic/get",
			        data:myData,
			        dataType: "json",
			        async: true,
			        success:function(data){
						...
						$.each(data,function(){
							...
							reimbursementCostTotal = reimbursementCostTotal + this.reimbursementCostSum;
							peopleNoTotal = peopleNoTotal + this.peopleNo;
						});
			        	if($("#table").html()==""){
							$("#table").html("没有查询到结果");
						}else{
							table.append('<tr><td>合计：'+data.length+' 个团</td><td></td><td></td><td></td><td>'+willIncomeTotal.toFixed(2)+'</td><td>'+realIncomeTotal.toFixed(2)+'</td><td>'+notIncomeTotal.toFixed(2)+'</td><td>'+reimbursementCostTotal.toFixed(2)+'</td><td>'+(willIncomeTotal-reimbursementCostTotal).toFixed(2)+'</td><td>'+((willIncomeTotal-reimbursementCostTotal)/willIncomeTotal*100).toFixed(2)+'%</td><td>'+peopleNoTotal+'</td><td></td><td></td></tr>');
						}
			         }
				});
			}
